...
enum OPCODE
{
  ...
  cDeg,
  cRad, /* Multiplication and division by 180 / pi */
...
struct NamePtr
{
  ...
  {
    return nameLength == rhs.nameLength && std::memcmp(name, rhs.name, nameLength) == 0;
  }
  inline bool operator<(const NamePtr & rhs) const
  {
...
template <typename Value_t>
class NamePtrsMap
  : public std::map<FUNCTIONPARSERTYPES::NamePtr, FUNCTIONPARSERTYPES::NameData<Value_t>>
{
};
