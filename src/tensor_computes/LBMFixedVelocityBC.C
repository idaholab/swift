...
LBMFixedVelocityBCTempl<dimension>::validParams()
{
  ...
  params.addRequiredParam<double>("velocity", "Fixed input velocity");
  return params;
}
...
{
  torch::Tensor density = 1.0 / (1.0 - _velocity) * (_f.index({0, Slice(), Slice(), 0}) + 
  ...
  _u.index_put_({0, Slice(), Slice(), _stencil._left[0]}, _f.index({0, Slice(), Slice(), opposite_dir}) + 2.0/3.0 * density * _velocity);
...
{ 
  torch::Tensor density = 1.0 / (1.0 + _velocity) * (_f.index({_grid_size[0] - 1, Slice(), Slice(), 0}) + 
  ...
  // axis aligned direction
  const auto & opposite_dir = _stencil._op[_stencil._left[0]];
  _u.index_put_({_grid_size[0] - 1, Slice(), Slice(), opposite_dir}, _f.index({_grid_size[0] - 1, Slice(), Slice(), _stencil._left[0]}) - 2.0/3.0 * density * _velocity);
...
{
  torch::Tensor density = 1.0 / (1.0 - _velocity) * (_f.index({Slice(), 0, Slice(), 0}) + 
                        _f.index({Slice(), 0, Slice(), 1}) + 
                        _f.index({Slice(), 0, Slice(), 3}) + 
                        2 * (_f.index({Slice(), 0, Slice(), 4}) + 
                        _f.index({Slice(), 0, Slice(), 7}) + 
                        _f.index({Slice(), 0, Slice(), 8}))) / _velocity;
  
  // axis aligned direction
  const auto & opposite_dir = _stencil._op[_stencil._front[0]];
  _u.index_put_({Slice(), 0, Slice(), _stencil._front[0]}, _f.index({Slice(), 0, Slice(), opposite_dir}) + 2.0/3.0 * density * _velocity);
...
{
  torch::Tensor density = 1.0 / (1.0 + _velocity) * (_f.index({Slice(), _grid_size[1] - 1, Slice(), 0}) + 
  ...
  // axis aligned direction
  const auto & opposite_dir = _stencil._op[_stencil._front[0]];
  _u.index_put_({Slice(), _grid_size[1] - 1, Slice(), opposite_dir}, _f.index({Slice(), _grid_size[1] - 1, Slice(), _stencil._front[0]}) - 2.0/3.0 * density * _velocity);
