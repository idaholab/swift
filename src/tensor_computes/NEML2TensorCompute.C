  ...
  : TensorOperatorBase(params)
    ...
    _model(
        [this]()
        {
          ...
          return std::ref(NEML2Utils::getModel(model_name, MooseTensor::floatTensorOptions().device()));
        }())
#endif
{
  ...
  {
    const auto * input_buffer = &getInputBuffer<>(swift_input_name);
    _input_mapping.emplace_back(input_buffer, neml2::LabeledAxisAccessor(neml2_input_name));
  }

  for (const auto & [neml2_output_name, swift_output_name] :
       getParam<std::string, TensorInputBufferName>("neml2_outputs", "swift_outputs"))
  {
    auto * output_buffer = &getOutputBuffer<>(swift_output_name);
    _output_mapping.emplace_back(neml2::LabeledAxisAccessor(neml2_output_name), output_buffer);
  }
#endif
}
